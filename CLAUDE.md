...
- **Frontend:** Vite 6 + React 19 + TypeScript 5.7 + Tailwind v4
  (`@tailwindcss/vite`, `@theme` tokens) + `vite-plugin-pwa` (service worker +
  manifest) + Vitest, under `web/`. Package manager **pnpm 11**.
...
Each row also carries `selection` (`{first, latest: {at, price}, selected}`,
null if never picked) merged in by `write_json` from the ledger.
...
- `selections.py` — the selection ledger (pure): a stock is **picked** when it
  passes >= 2 screens; keeps its first-ever entry (run + price, never
  overwritten) and its latest re-entry after a drop-out. Pass gates mirror
  `web/src/score.ts` (a test diffs them). `backfill_selections.py` rebuilt it
  from the 27 recoverable published runs (2026-08-20 on, SHAs inside).
...
- `web/src/` — SPA. `score.ts` (verdicts — Azqato/Lynch/Graham gate only,
  ...
  `localStorage`; the query is not saved), `selection.ts` (entry price /
  move since, for card + scorecard), `StockCard.tsx`, `FilterBar.tsx`, `Header.tsx` (freshness chip), `Toasts.tsx` (new-version /
...
- `.github/workflows/ci-frontend.yml` — on `web/**` change: typecheck + unit tests + build.
...
- **Typecheck:** `pnpm -C web run typecheck`. **Tests:** `pnpm -C web test`.
